...
func initLogger(cfg TelemetryConfig) *zap.Logger {
    zapCfg := zap.NewProductionConfig()
    zapCfg.Level = zap.NewAtomicLevelAt(zap.InfoLevel)
    logger, _ := zapCfg.Build(
        zap.WithCaller(true),
        // 可选：包装为 OTel Core，自动注入 trace_id / span_id
    )
    return logger
}
```
...
logger.Info("tag applied",
    ...
    zap.String("agent_template", template),
    zap.String("trace_id", span.SpanContext().TraceID().String()),
)

// 禁止
log.Println("tag applied: " + taskID)  // 非结构化
...
test_isolation_strategy:
  ...
  integration_tests:
    policy: "dockertest（每测试一套新实例）"
    implementation: |
      pool, _ := dockertest.NewPool("")
      pgResource, _ := pool.Run("postgres", "15", []string{"POSTGRES_PASSWORD=test"})
      redisResource, _ := pool.Run("redis", "7", nil)
